...
> 界面由 v1.3.1 程序真实渲染，使用固定演示数据，不代表任何真实账户。

## 界面预览

以下主题图全部由最终 v1.3.1 程序自身的界面绘制代码重新导出，不是 AI 生成图；其中的数值均为固定演示数据。
...
- 右键主条中的 `PRO｜周用量剩余…` 正文可直接退出工具；雷达状态、任务状态、齿轮和顶部预告窗不会触发退出。
...
1. 从 [v1.3.1 Release](https://github.com/ivan51769/CodexUsageOverlay/releases/tag/v1.3.1) 下载 `CodexUsageOverlay-Setup-1.3.1.exe`。
...
- 取消：放弃本次修改。
- 退出工具：右键主条中的 `PRO｜周用量剩余…` 正文，或展开设置后单击“退出工具”；再次运行桌面快捷方式即可恢复。
