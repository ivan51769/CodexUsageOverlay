...
一个面向 Windows Codex 桌面应用的轻量用量与更新助手。它跟随 Codex 窗口显示账户套餐、周用量剩余、重置时间、可用重置券、累计 Token、当前任务状态，以及基于公开非官方来源整理的 Tibo 重置预告；也可从悬浮条直接打开 Codex 桌面版 MSIX 更新助手。

设置面板中的公众号名与作者署名在所有主题下默认使用渐变彩字。

### 原生分析数据

分析入口直接读取 Codex 原生客户端使用的官方 `/wham/usage/daily-token-usage-breakdown` 和 `/wham/analytics/daily-*` 接口，展示产品/模型用量、模型/使用端轮次、技能/插件调用。7 天和 30 天均按 UTC 截止昨天；缺失日期不补零，单位沿用接口返回值，不将百分比或轮次解释成 Token。技能与插件保留原生 Top 10 / Other。

认证只读取 `CODEX_HOME/auth.json`（未配置时为用户 `.codex/auth.json`）中的现有登录态，仅在内存中用于固定 `https://chatgpt.com/backend-api/wham/` 请求，禁止重定向，不保存凭据、不刷新或改写共享登录文件、不上传本地对话。支持 `HTTPS_PROXY`。需要本机已有 Codex 登录态；过期时应在 Codex 重新登录。上述客户端内部接口并非稳定公开 SDK，变化或不可用时会显示错误而非伪造数据。
...
以下为 v1.4.29 程序实际绘制的空数据/离线预览，未展示真实账户用量。

![原生分析面板](docs/images/releases/v1.4.29/native-analysis.png)
![完整版下载器（保留公众号 Logo）](docs/images/releases/v1.4.29/full-updater.png)
![展开式下载面板](docs/images/releases/v1.4.29/download-panel.png)
...
- 兼容 Tibo 状态源的 reset-bank 与高概率重置预告说明格式；单次网络失败时保留有效缓存并每 60 秒重试，超过 30 小时未更新才显示“雷达离线”。
